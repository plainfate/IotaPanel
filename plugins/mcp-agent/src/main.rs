...
fn rand_hex(len: usize) -> String {
    ...
    let mut s = RandomState::new().build_hasher();
    s.write_u64(std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).map(|d| d.as_nanos() as u64).unwrap_or(0));
    let mut out = String::with_capacity(len * 2);
    for _ in 0..len {
        s.write_u64(s.finish().wrapping_mul(0x9E3779B97F4A7C15));
        let b = (s.finish() & 0xff) as u8;
