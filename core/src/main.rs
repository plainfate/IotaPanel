...
fn serve() {
    ...
    let listener = match bind_listener(&srv.cfg.lock().unwrap().listen_addr) {
        Ok(l) => l,
        Err(e) => {
            let addr = srv.cfg.lock().unwrap().listen_addr.clone();
            manager::log_line("ERROR", &format!("监听失败 addr={} err={}", addr, e));
            std::process::exit(1);
        }
    };
